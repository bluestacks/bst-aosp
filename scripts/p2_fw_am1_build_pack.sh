#!/bin/bash
set +u
LOG=~/p2_fw_am1_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-AM-1 build start $(date -Is)"
cd ~/aosp16 || exit 1
...
# ensure DisplayContent WM-2 still reverted
rg -n 'A16DBG:P2:FW-WM-2' frameworks/base/services/core/java/com/android/server/wm/DisplayContent.java && {
  echo "FATAL: WM-2 still in tree"; exit 3
} || echo "WM-2 absent OK"
m services -j24
rc=$?
echo "services_rc=$rc"
...
echo "A16DBG:P2:FW-AM-1 DONE $(date -Is)"
