#!/bin/bash
# P2-FW-SERVICES-2a: IntentResolver + ComponentResolver apply + Layer1 + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_services2a_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-SERVICES-2a build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-SERVICES-2a' \
    "$A16/frameworks/base/services/core/java/com/android/server/IntentResolver.java" \
    2>/dev/null; then
  python3 "$BST/scripts/p2_fw_services2a_apply.py" || exit 1
else
...
git diff HEAD -- \
  ...
  > "$PATCH_DIR/P2-FW-SERVICES-2a.diff" || true

echo "A16DBG:P2:FW-SERVICES-2a DONE $(date -Is)"
