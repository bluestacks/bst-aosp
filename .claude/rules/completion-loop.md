...
1. **Validate** — `validation-gate.md` 的两层（Layer 1 远程编译 + Layer 2 启动 readback）。时序：**产品板 / G10 打包等 boot 关键组 Layer 2 强制**；当前 Windows gate 使用 `android_x86_64`。其余组 Layer 1 先行，Layer 2 在并入 boot 镜像时跑。按 readback 验证，别假设改对了。
