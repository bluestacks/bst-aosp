#!/bin/bash
# Round 104: narrow odsign super-key bringup bypass after rebind allow
set -e
AOSP=~/aosp16
...
t = sec.read_text()
marker = "BS bringup: store odsign early-boot key without boot-stage super encryption"
old = '''                    let (key_blob, mut blob_metadata) = SUPER_KEY
                        .read()
                        .unwrap()
...
'''
new = '''                    let (key_blob, mut blob_metadata) = match SUPER_KEY
                        .read()
                        .unwrap()
                        .handle_super_encryption_on_key_init(
                            &mut db,
                        ...
                        Ok(v) => v,
                        Err(e) => {
                            let odsign_boot_key = key.domain == Domain::SELINUX
                                && key.nspace == 101
...
if marker not in t:
    if old not in t:
        raise SystemExit("security_level super encryption anchor not found")
    sec.write_text(t.replace(old, new, 1))
    print("keystore2 security_level patched: odsign boot-stage super-key bypass")
else:
    print("keystore2 security_level patch already present")
PY
