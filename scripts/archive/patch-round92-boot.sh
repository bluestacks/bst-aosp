#!/bin/bash
# Round 92: boot-image.prof + framework bind + odsign.verification.success + dalvik.vm.boot-image
set -e
...
# Ensure initrd.img packs profiles/ (Makefile has dalvik-cache rule only)
MK="$BOOT/Makefile"
