#!/bin/bash
# FW-CORE-APP-14: NativeLibraryHelper apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_coreapp14_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-CORE-APP-14 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-CORE-APP-14' \
    "$A16/frameworks/base/core/java/com/android/internal/content/NativeLibraryHelper.java" 2>/dev/null; then
  python3 "$BST/scripts/p2_fw_coreapp14_apply.py" || exit 1
else
...
git diff HEAD -- core/java/com/android/internal/content/NativeLibraryHelper.java \
  > "$BST/patches/android-16/patches/p2-framework-rest/P2-FW-CORE-APP-14.diff" || true

echo "A16DBG:P2:FW-CORE-APP-14 DONE $(date -Is)"
