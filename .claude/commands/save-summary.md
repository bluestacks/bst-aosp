...
- **Stage identity** — `aosp16-development` / `android16-promotion` /
  `android16-mainline`；resolved tree、branch、完整 HEAD、OUT_DIR、product。
- **Multi-repo identity** — 根 SHA、受影响组件 SHA、根 gitlink、
  submodule audit 结果和目标 remote/branch。
...
- **如何验证** — 哪些 gate 命令跑了且过：Layer 1（命令 + exit code +
  产物路径 + mtime + SHA-256 + identity HEAD）、Layer 2（哪些 oracle 跑了、
  结果片段及部署 identity）。用 readback 证据表述，不用「应该行」。
  boot-pending 显式标注。
