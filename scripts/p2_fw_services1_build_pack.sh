#!/bin/bash
# P2-FW-SERVICES-1: ClipboardService + LocationManagerService apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_services1_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-SERVICES-1 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-SERVICES-1' \
    "$A16/frameworks/base/services/core/java/com/android/server/clipboard/ClipboardService.java" \
    2>/dev/null; then
  python3 "$BST/scripts/p2_fw_services1_apply.py" || exit 1
else
...
git diff HEAD -- \
  ...
  > "$PATCH_DIR/P2-FW-SERVICES-1.diff" || true

echo "A16DBG:P2:FW-SERVICES-1 DONE $(date -Is)"
