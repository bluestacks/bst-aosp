...
cleanup_stale_rootfs_artifacts() {
    ...
    sudo rm -rf "$OUT/rootFSDebug" "$OUT/rootFS" "$OUT/dataFS"
    sudo rm -f "$OUT/Root.fs" "$OUT/Root.fs.debug" "$OUT/Root.fs.rooted"
    sudo rm -rf "$OUT/rooted_system" "$OUT/rooted_root" "$OUT/system" "$OUT/root"
}
...
baklava_build() {
    ...
    export ENABLE_DEXOPT="${ENABLE_DEXOPT:-false}"
    export WITHOUT_CHECK_API="${WITHOUT_CHECK_API:-true}"
    export BUILD_FROM_SOURCE_STUB="${BUILD_FROM_SOURCE_STUB:-true}"
