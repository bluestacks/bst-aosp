#!/bin/bash
# FW-CORE-APP-4: apply + Layer1 m droid + pack (clouddev).
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_coreapp4_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-CORE-APP-4 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-CORE-APP-4' "$A16/frameworks/base/core/java/android/view/ViewRootImpl.java" 2>/dev/null; then
  python3 "$BST/scripts/p2_fw_coreapp4_apply.py" || exit 1
else
...
cd "$A16/frameworks/base" || exit 1
git diff HEAD -- \
  core/java/android/view/ViewRootImpl.java \
  core/java/android/view/InputDevice.java \
  > "$BST/patches/android-16/patches/p2-framework-rest/P2-FW-CORE-APP-4.diff" || true

echo "A16DBG:P2:FW-CORE-APP-4 DONE $(date -Is)"
