#!/bin/bash
# P2-FW-SERVICES-3: AccountManagerService apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_services3_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-SERVICES-3 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-SERVICES-3' \
    "$A16/frameworks/base/services/core/java/com/android/server/accounts/AccountManagerService.java" \
    2>/dev/null; then
  python3 "$BST/scripts/p2_fw_services3_apply.py" || exit 1
else
...
git diff HEAD -- \
  ...
  > "$PATCH_DIR/P2-FW-SERVICES-3.diff" || true

echo "A16DBG:P2:FW-SERVICES-3 DONE $(date -Is)"
