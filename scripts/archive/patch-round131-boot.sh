#!/bin/bash
# Round 131: enrich art-libs with libarttools.so etc from payload before bind (odrefresh linker)
set -e
AOSP=~/aosp16
...
cd "$BOOT"
echo "stage2 7BP=$(grep -c 'henry-7BP art-libs payload-enrich' stage2.sh || true)"
