#!/bin/bash
# FW-CORE-APP-11: ResourcesImpl apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_coreapp11_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-CORE-APP-11 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-CORE-APP-11' \
    "$A16/frameworks/base/core/java/android/content/res/ResourcesImpl.java" 2>/dev/null; then
  python3 "$BST/scripts/p2_fw_coreapp11_apply.py" || exit 1
else
...
git diff HEAD -- core/java/android/content/res/ResourcesImpl.java \
  > "$BST/patches/android-16/patches/p2-framework-rest/P2-FW-CORE-APP-11.diff" || true

echo "A16DBG:P2:FW-CORE-APP-11 DONE $(date -Is)"
