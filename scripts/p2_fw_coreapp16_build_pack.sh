#!/bin/bash
# FW-CORE-APP-16: ActivityThread apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_coreapp16_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-CORE-APP-16 build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-CORE-APP-16' \
    "$A16/frameworks/base/core/java/android/app/ActivityThread.java" 2>/dev/null; then
  python3 "$BST/scripts/p2_fw_coreapp16_apply.py" || exit 1
else
...
git diff HEAD -- core/java/android/app/ActivityThread.java \
  > "$BST/patches/android-16/patches/p2-framework-rest/P2-FW-CORE-APP-16.diff" || true

echo "A16DBG:P2:FW-CORE-APP-16 DONE $(date -Is)"
