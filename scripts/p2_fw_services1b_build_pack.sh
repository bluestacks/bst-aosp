#!/bin/bash
# P2-FW-SERVICES-1b: LocationManagerService only apply + Layer1 m droid + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_services1b_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-SERVICES-1b build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-SERVICES-1b' \
    "$A16/frameworks/base/services/core/java/com/android/server/location/LocationManagerService.java" \
    2>/dev/null; then
  python3 "$BST/scripts/p2_fw_services1b_apply.py" || exit 1
else
...
git diff HEAD -- \
  ...
  > "$PATCH_DIR/P2-FW-SERVICES-1b.diff" || true

echo "A16DBG:P2:FW-SERVICES-1b DONE $(date -Is)"
