#!/bin/bash
# Round 113: do not let inner odsign kill the wrapper before stderr/readback logs
set -e
AOSP=~/aosp16
...
t = p.read_text()
old = 'constexpr const char* kStopServiceProp = "ctl.stop";'
new = 'constexpr const char* kStopServiceProp = "bs.odsign.stop";  // BS bringup: keep wrapper alive for readback'
if 'kStopServiceProp = "bs.odsign.stop"' not in t:
    if old not in t:
        raise SystemExit("odsign stop property anchor not found")
    t = t.replace(old, new, 1)
    p.write_text(t)
    print("odsign patched: disable ctl.stop self-kill")
else:
    print("odsign ctl.stop disable patch already present")
PY
