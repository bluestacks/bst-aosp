#!/bin/bash
# P2-FW-SERVICES-2b: NMS only apply + Layer1 + pack.
set -eo pipefail
A16=~/aosp16
BST=~/bst-aosp
LOG=~/p2_fw_services2b_build.log
exec > >(tee "$LOG") 2>&1
echo "A16DBG:P2:FW-SERVICES-2b build start $(date -Is)"

if ! rg -q 'A16DBG:P2:FW-SERVICES-2b' \
    "$A16/frameworks/base/services/core/java/com/android/server/notification/NotificationManagerService.java" \
    2>/dev/null; then
  python3 "$BST/scripts/p2_fw_services2b_apply.py" || exit 1
else
...
git diff HEAD -- \
  ...
  > "$PATCH_DIR/P2-FW-SERVICES-2b.diff" || true

echo "A16DBG:P2:FW-SERVICES-2b DONE $(date -Is)"
